...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: "All Course", path: "/course/all" },
    ],
  },
  {
    name: "Payment",
...
const AppSidebar: React.FC = () => {
  ...
  const renderMenuItems = (items: NavItem[], menuType: "main" | "others") => (
    <ul className="flex flex-col gap-4">
      {items.map((nav, index) => (
        <li
          key={nav.name}
          onClick={() => !nav.subItems && mobileHandleToggle()}
        >
          {nav.subItems ? (
            <button
            ...
            >
              ...
              >
                <i style={{ fontSize: "24px" }} className={nav.icon}></i>
              </span>
              ...
              {(isExpanded || isHovered || isMobileOpen) && (
                <i
                  className={`fas fa-chevron-down ml-auto w-5 h-5 transition-transform duration-200 ${openSubmenu?.type === menuType &&
            ...
            >
              <ul className="mt-2 space-y-1 ml-9">
                {nav.subItems.map((subItem) => (
                  <li key={subItem.name} onClick={mobileHandleToggle}>
                    <Link
                      to={subItem.path}
  ...
  return (
    ...
    >
      <div
        className={`py-8 hidden md:flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"
          }`}
      >
        <Link to="/">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <h1 className="font-semibold text-2xl">Shorif <span className="text-blue-500">Art School</span></h1>
            </>
          ) : (
            <img
              src="/favicon.jpg"
